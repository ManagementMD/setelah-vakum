...
const connectToWhatsApp = async () => {
	...
	let metadata = await conn.groupMetadata(data.id)
	  for (let i of data.participants) {
		...
		if (data.action == "add") {
		  var welcomenya = await getBuffer(`http://hadi-api.herokuapp.com/api/card/Welcome?nama=${i.split("@")[0]}&descriminator=KIRO-BOT&memcount=${metadata.participants.length}&gcname=${metadata.subject}&pp=${pp_user}&bg=https://i.ibb.co/98m53bv/welcome.jpg`)
